...
os.makedirs("data/dependency-trees", exist_ok=True)
for lang in LANGUAGES:
    print(f'Processing {lang.name}...')
    src_file = f"data/conllu/{lang.name}.conllu"
    dst_file = f"data/dependency-trees/{lang.name}.json"
    with open(src_file, "r", encoding="utf-8") as f:
        conllu_data = f.readlines()
    # Process the CoNLL-U data and generate dependency trees
    trees = []
    for line in conllu_data:
        ...
        if len(columns) != Cols.TOTAL:
            ...
            continue
        assert trees, f"Data error in {lang.name}: token line before sentence text"
        id = columns[Cols.ID]
